...
// Add code here
  ...
  public static String translate(String input)
  {
    ...
    for(int i =0; i< result.length(); i++){
      if(result.substring(i, i+1).equals(" ")|| result.substring(i, i+1).equals("-")|| result.substring(i, i+1).equals(".")){
    ...
    if(result.length()>0){
      for(int i=0;i<result.length(); i++){
        if((result.charAt(i)== 'a'|| result.charAt(i)== 'e'|| result.charAt(i)== 'i'|| result.charAt(i)== 'o'|| result.charAt(i)== 'u')|| (result.charAt(i)== 'A'|| result.charAt(i)== 'E'|| result.charAt(i)== 'I'|| result.charAt(i)== 'O'|| result.charAt(i)== 'U')){
          result = result.substring(i, i+1).toUpperCase()+result.substring(i+1) + result.substring(0, i).toLowerCase();
          return (result+="ay");
           }
        }
